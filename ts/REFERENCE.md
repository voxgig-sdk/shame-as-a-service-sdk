# ShameAsAService TypeScript SDK Reference

Complete API reference for the ShameAsAService TypeScript SDK.
...
```ts
new ShameAsAServiceSDK(options?: object)
```
...
#### `ShameAsAServiceSDK.test(testopts?, sdkopts?)`
...
**Returns:** `ShameAsAServiceSDK` instance in test mode.


### Instance Methods

#### `GetShameMessage(data?: object)`

Create a new `GetShameMessage` entity instance.
...
**Returns:** `GetShameMessageEntity` instance.
...
Alias for `ShameAsAServiceSDK.test()`.
...
```ts
const result = await client.GetShameMessage().load()
```
...
Return the parent `ShameAsAServiceSDK` instance.
