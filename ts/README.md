# ShameAsAService TypeScript SDK



The TypeScript SDK for the ShameAsAService API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.GetShameMessage()` — each with a small set of operations (`load`)
...
| `tester(testopts?, sdkopts?)` | `ShameAsAServiceSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `ShameAsAServiceSDK.test(testopts?, sdkopts?)` | `ShameAsAServiceSDK` | Create a test-mode client. |
...
| `client` | `client(): ShameAsAServiceSDK` | Return the parent SDK client. |
...
```ts
const get_shame_message = await client.GetShameMessage().load()
```
...
shame-as-a-service/
├── src/
│   ├── ShameAsAServiceSDK.ts        # Main SDK class
