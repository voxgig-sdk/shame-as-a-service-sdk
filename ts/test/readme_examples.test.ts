...
//     (\`new ShameAsAServiceSDK(...)\` / \`ShameAsAServiceSDK.test(...)\` become
//     \`ShameAsAServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ShameAsAServiceSDK'
...
const TEST_SEED = {"entity":{"get_shame_message":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ShameAsAServiceSDK(...)\` and any
// \`ShameAsAServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ShameAsAServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ShameAsAServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
