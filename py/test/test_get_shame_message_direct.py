...
class TestGetShameMessageDirect:

    def test_should_direct_load_get_shame_message(self):
        setup = _get_shame_message_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get_shame_message", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_shame_message_direct_setup(mockres):
