# ShameAsAService SDK test runner
...
class ShameAsAServiceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ShameAsAServiceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ShameAsAServiceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ShameAsAServiceTestRunner.getenv("SHAME_AS_A_SERVICE_TEST_LIVE")
        override = ShameAsAServiceTestRunner.getenv("SHAME_AS_A_SERVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ShameAsAServiceTestRunner.getenv(key)
        ...
        explain = ShameAsAServiceTestRunner.getenv("SHAME_AS_A_SERVICE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SHAME_AS_A_SERVICE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ShameAsAServiceTestRunner._test_control is not None:
            return ShameAsAServiceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ShameAsAServiceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ShameAsAServiceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ShameAsAServiceTestRunner.load_test_control()
...
def load_env_local():
    ShameAsAServiceTestRunner.load_env_local()


def env_override(m):
    return ShameAsAServiceTestRunner.env_override(m)


def entity_data(v):
    return ShameAsAServiceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ShameAsAServiceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ShameAsAServiceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ShameAsAServiceTestRunner.load_test_control()


def live_client_options():
    return ShameAsAServiceTestRunner.live_client_options()


def live_delay_ms():
    return ShameAsAServiceTestRunner.live_delay_ms()
