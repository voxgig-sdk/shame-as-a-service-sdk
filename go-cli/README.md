# shame-as-a-service-cli

boru-driven command-line client **and** interactive REPL for the ShameAsAService
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/shame-as-a-service-cli)
make build

# 2. See usage (words, entities, env vars)
./shame-as-a-service-cli --help

# 3. Provide credentials once, via the environment
export SHAME_AS_A_SERVICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./shame-as-a-service-cli load 1 get_shame_message            # {id:1} shorthand
./shame-as-a-service-cli load '{id:1}' get_shame_message       # explicit match map

# 5. Override the API base URL for a single call
SHAME_AS_A_SERVICE_BASE=https://api.example.com ./shame-as-a-service-cli load 1 get_shame_message

# 6. No arguments -> interactive REPL
./shame-as-a-service-cli
shame-as-a-service> load 1 get_shame_message
shame-as-a-service> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/shame-as-a-service-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/shame-as-a-service-cli load 1 get_shame_message
   ```
...
```sh
./shame-as-a-service-cli load 1 get_shame_message          # scalar shorthand for {id:1}
./shame-as-a-service-cli load '{id:1}' get_shame_message     # explicit match map
```
...
./shame-as-a-service-cli load 1 get_shame_message
```
...
Run with no arguments to open a REPL (prompt `shame-as-a-service>`). Each line is
evaluated as its own boru expression:

```text
$ ./shame-as-a-service-cli
shame-as-a-service> load 1 get_shame_message
shame-as-a-service> /help
shame-as-a-service> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_shame_message`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/shame-as-a-service-cli`. |
...
(`./shame-as-a-service-cli <expr>`) and interactively (the REPL), and expressions compose the
