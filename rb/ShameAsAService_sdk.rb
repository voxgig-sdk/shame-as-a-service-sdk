...
require_relative 'ShameAsAService_types'
...
class ShameAsAServiceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ShameAsAServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ShameAsAServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ShameAsAServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ShameAsAServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ShameAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ShameAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ShameAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ShameAsAServiceError => err
    ...
    if fetched.is_a?(Hash)
      status = ShameAsAServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ShameAsAServiceError.new(
        "graphql_error", "ShameAsAServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ShameAsAServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
