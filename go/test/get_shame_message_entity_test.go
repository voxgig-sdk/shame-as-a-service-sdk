...
func TestGetShameMessageEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SHAMEASASERVICE_TEST_GET_SHAME_MESSAGE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		getShameMessageRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.get_shame_message", setup.data)))
...
func get_shame_messageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SHAMEASASERVICE_TEST_GET_SHAME_MESSAGE_ENTID")
