...
import (
	...
	omni "github.com/voxgig-sdk/shame-as-a-service-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ShameAsAServiceError); is {
				out := map[string]any{
					"name":    "ShameAsAServiceError",
...
func omniCtx(arg any, client *sdk.ShameAsAServiceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ShameAsAServiceSDK); is {
			client = live
