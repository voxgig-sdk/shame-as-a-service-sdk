...
module ShameAsAServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ShameAsAServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = ShameAsAServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ShameAsAServiceResponse.new(fetched)
    else
