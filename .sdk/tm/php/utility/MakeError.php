...
class ShameAsAServiceMakeError
{
    public static function call(?ShameAsAServiceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ShameAsAServiceContext([], null);
        }
        $op = $ctx->op ?? new ShameAsAServiceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ShameAsAServiceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ShameAsAServiceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ShameAsAServiceError('', $msg, $ctx);
        ...
        if ($err instanceof ShameAsAServiceError) {
