...
// ShameAsAService SDK pipeline test
//
...
{
    private static function utility(): ShameAsAServiceUtility
    {
        return new ShameAsAServiceUtility();
    }
    ...
    private static function ctx(array $over = []): ShameAsAServiceContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ShameAsAServiceContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ShameAsAServiceOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ShameAsAServiceError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ShameAsAServiceOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ShameAsAServiceMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ShameAsAServiceOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ShameAsAServiceMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ShameAsAServiceOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ShameAsAServiceMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShameAsAServiceSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ShameAsAServiceMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ShameAsAServiceResponse([]),
            'result' => new ShameAsAServiceResult([]),
        ]);
        [, $err] = ShameAsAServiceMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ShameAsAServiceSpec([]),
            'response' => null,
            'result' => new ShameAsAServiceResult([]),
        ]);
        [, $err] = ShameAsAServiceMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ShameAsAServiceSpec(['step' => 's']),
            'response' => new ShameAsAServiceResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ShameAsAServiceResult([]),
        ]);
        [, $err] = ShameAsAServiceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ShameAsAServiceSpec(['step' => 's']),
            'response' => new ShameAsAServiceResponse(self::resp(200, ['v' => 1])),
            'result' => new ShameAsAServiceResult([]),
        ]);
        [, $err] = ShameAsAServiceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ShameAsAServiceSpec(['step' => 's']),
            'response' => new ShameAsAServiceResponse(self::resp(200, ['v' => 2])),
            'result' => new ShameAsAServiceResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ShameAsAServiceMakeResponse::call($ctx);
    ...
    {
        $preset = new ShameAsAServiceResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ShameAsAServiceSpec([]),
            'response' => new ShameAsAServiceResponse([]),
            'result' => new ShameAsAServiceResult([]),
        ...
        [$got, $err] = ShameAsAServiceMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ShameAsAServiceResult([])]);
        [, $err] = ShameAsAServiceMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ShameAsAServiceSpec([]), 'result' => null]);
        [, $err] = ShameAsAServiceMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShameAsAServiceSpec(['step' => 's']),
            'result' => new ShameAsAServiceResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ShameAsAServiceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShameAsAServiceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShameAsAServiceSpec(['step' => 's']),
            'result' => new ShameAsAServiceResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ShameAsAServiceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShameAsAServiceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShameAsAServiceResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ShameAsAServiceSpec([]),
            'result' => new ShameAsAServiceResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ShameAsAServiceMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShameAsAServiceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ShameAsAServiceError('boom', 'boom');
        $utility->fetcher = function (ShameAsAServiceContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShameAsAServiceSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ShameAsAServiceMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ShameAsAServiceContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ShameAsAServiceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ShameAsAServiceResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ShameAsAServiceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ShameAsAServiceContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShameAsAServiceSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ShameAsAServiceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ShameAsAServiceResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ShameAsAServiceSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ShameAsAServiceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ShameAsAServiceMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ShameAsAServiceMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ShameAsAServiceResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ShameAsAServiceDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ShameAsAServiceResult(['ok' => false])]);
        $this->expectException(ShameAsAServiceError::class);
        ShameAsAServiceDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ShameAsAServiceResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ShameAsAServiceMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ShameAsAServiceMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ShameAsAServiceBaseFeature();
        $b = new ShameAsAServiceBaseFeature();
        ShameAsAServiceFeatureAdd::call($ctx, $a);
        ShameAsAServiceFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ShameAsAServiceBaseFeature
    {
        $f = new ShameAsAServiceBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ShameAsAServiceFeatureAdd::call($ctx, self::named_feature('a'));
        ShameAsAServiceFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ShameAsAServiceFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ShameAsAServiceFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ShameAsAServiceFeatureAdd::call($ctx, $replace);
        ...
        ShameAsAServiceFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ShameAsAServiceContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ShameAsAServiceSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShameAsAServicePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ShameAsAServicePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ShameAsAServiceResponse(['headers' => null]),
            'result' => new ShameAsAServiceResult([]),
        ]);
        ShameAsAServiceResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ShameAsAServiceResultBody::call($ctx);
