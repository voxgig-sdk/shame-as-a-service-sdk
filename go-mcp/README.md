# shame-as-a-service-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ShameAsAService SDK as
two agent tools — `shame-as-a-service_list` and `shame-as-a-service_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/shame-as-a-service-mcp)
make build

# 2. Provide credentials via the environment
export SHAME_AS_A_SERVICE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user shame-as-a-service \
  -- /absolute/path/to/shame-as-a-service-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./shame-as-a-service-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// shame-as-a-service_list: first page of records
{ "entity": "get_shame_message" }
{ "entity": "get_shame_message", "query": { } }

// shame-as-a-service_load: one record by id
{ "entity": "get_shame_message", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/shame-as-a-service-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user shame-as-a-service \
     -- "$PWD"/dist/*/shame-as-a-service-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `shame-as-a-service_list` and `shame-as-a-service_load` tools now appear
   in new sessions. Ask the agent to *"list get_shame_message using shame-as-a-service"*
   and it calls `shame-as-a-service_list` with `{"entity":"get_shame_message"}`.
...
```sh
export SHAME_AS_A_SERVICE_APIKEY=sk_live_xxx            # API key
export SHAME_AS_A_SERVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `shame-as-a-service_list` tool
...
### Call the `shame-as-a-service_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `shame-as-a-service_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `shame-as-a-service_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SHAME_AS_A_SERVICE_APIKEY` | API key sent with every request. |
| `SHAME_AS_A_SERVICE_BASE` | Optional override of the API base URL. |
...
```sh
./shame-as-a-service-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"shame-as-a-service_load","arguments":{"entity":"get_shame_message","query":{"id":1}}}}'
```
