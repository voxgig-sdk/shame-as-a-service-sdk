...
class ShameAsAServiceSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ShameAsAServiceHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ShameAsAServiceHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ShameAsAServiceFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ShameAsAServiceFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ShameAsAServiceUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ShameAsAServiceHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ShameAsAServiceHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ShameAsAServiceHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ShameAsAServiceError($op . "_allow",
                "ShameAsAServiceSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ShameAsAServiceHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ShameAsAServiceError("graphql_error",
                "ShameAsAServiceSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetShameMessage($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_shame_message === null) {
                $this->_get_shame_message = new GetShameMessageEntity($this, null);
            }
            return $this->_get_shame_message;
        }
        return new GetShameMessageEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new ShameAsAServiceSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
