# ShameAsAService PHP SDK



The PHP SDK for the ShameAsAService API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->GetShameMessage()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
try {
    // load() returns the ENTITY — call data_get() for the GetShameMessage record (throws on error).
    $getshamemessage = $client->GetShameMessage()->load();
    print_r($getshamemessage->data_get());
} catch (\Throwable $err) {
...
```php
$client = ShameAsAServiceSDK::test($testopts, $sdkopts);
```
...
```
php/
├── shameasaservice_sdk.php          -- Main SDK class
...
The main class (`shameasaservice_sdk.php`) exports the SDK class
