...
// ShameAsAService SDK retry feature
...
class ShameAsAServiceRetryFeature extends ShameAsAServiceBaseFeature
{
    ...
    public function init(ShameAsAServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ShameAsAServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ShameAsAServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ShameAsAServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
