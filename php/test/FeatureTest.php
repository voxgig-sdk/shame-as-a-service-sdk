...
// ShameAsAService SDK feature test
//
...
class FtCtrl extends ShameAsAServiceControl
{
...
class FtClient
{
    ...
    private ShameAsAServiceUtility $utility;

    public function __construct(ShameAsAServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): ShameAsAServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ShameAsAServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ShameAsAServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ShameAsAServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ShameAsAServiceConfig::make_feature($name);
    ...
    private static function buildUrl(ShameAsAServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ShameAsAServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ShameAsAServiceResponse($fetched);
        }
    ...
    private function fail(ShameAsAServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ShameAsAServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
